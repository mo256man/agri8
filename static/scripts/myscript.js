...
$(async function() {
    ...
    $("#setClock").on("click", function(){
        const year = $("#year").val();
        const month = $("#month").val() -1;     // 月は0～11を設定するので-1する
        const day = $("#day").val();
        const hour = $("#hour").val();
        const minute = $("#minute").val();
        const now = dayjs(`${year}-${month}-${day} ${hour}:${minute}:00`).format("MMDDHHmmYYYY");
        alert(`sudo date '${now}'`);
...
async function do1st() {
    ...
    const ids = ["lat", "lon", "elev", "morning_offset", "evening_offset", "morning_minutes", "evening_minutes",
                 "sensing_interval", "sensing_count", "year", "month", "day", "hour", "minute", "yellow", "green"];    // キーパッドを設定するid
...
//    コンテック
//////////////////////////////////////////////////////////////////////
async function getContec(isTry, bool) {
    let msg = "";
